...
int main()
{
    ...
    ok = ok && ((x += y) == edu::Rational(3, 1));
    ok = ok && ((x -= y) == edu::Rational(1, 1));
    ok = ok && ((x *= y) == edu::Rational(2, 1));
    ok = ok && ((x /= y) == edu::Rational(1, 1));

    ok = ok && ((x++) == edu::Rational(1, 1));
    ok = ok && ((x--) == edu::Rational(2, 1));
    ok = ok && ((++y) == edu::Rational(3, 1));
    ok = ok && ((--y) == edu::Rational(2, 1));

    ok = ok && ((x + y) == edu::Rational(3, 1));
    ok = ok && ((x - y) == edu::Rational(-1, 1));
    ok = ok && ((x * y) == edu::Rational(2, 1));
    ok = ok && ((x / y) == edu::Rational(1, 2));
