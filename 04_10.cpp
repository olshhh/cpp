...
template <typename D>
constexpr bool is_empty_v = (size_v<D> == 0U);
...
// Non-empty deque specialization
template <typename T, typename H, typename... Ts>
struct Has<T, Deque<H, Ts...>>
{
    static constexpr bool value =
        std::is_same_v<T, H> || Has<T, Deque<Ts...>>::value;
};
...
int main()
{
    static_assert(size_v<Deque<>> == 0U && is_empty_v<Deque<>> == true);
    static_assert(size_v<Deque<int>> == 1U && is_empty_v<Deque<int>> == false);
    ...
    static_assert(std::is_same_v<nth<Deque<int, int>, 0U>, int>);
    static_assert(std::is_same_v<nth<Deque<int, int>, 1U>, int>);
